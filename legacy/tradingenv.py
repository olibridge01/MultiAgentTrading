...
class TradingEnvironment2(gym.Env):
    """
    A general trading environment for stock/forex trading using OpenAI Gym API.
    ...
    def __init__(
        ...
        super(TradingEnvironment2, self).__init__()
        assert render_mode is None or render_mode in self.metadata['render.modes']
    ...
    def _update_balance(self):
        """Update the balance based on the current position."""
        start_price = self.df.iloc[self._timestep]['Close']
        start_balance = self.balance
        
        end_price = self.df.iloc[self._timestep + 1]['Close']
        if self._position == 1:
            self.balance = start_balance * (end_price / start_price)
        elif self._position == -1:
            self.balance = start_balance * (start_price / end_price)
